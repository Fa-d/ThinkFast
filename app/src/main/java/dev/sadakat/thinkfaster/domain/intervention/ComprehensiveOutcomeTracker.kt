...
class ComprehensiveOutcomeTracker @Inject constructor(
    ...
    private suspend fun collectShortTermOutcome(
        ...
        val sessionContinued = session != null && !session.isEnded

        // Calculate session duration after intervention
        val sessionDurationAfter = if (session != null && session.isEnded) {
            val interventionTime = outcome.timestamp
            val sessionEndTime = session.endTime ?: System.currentTimeMillis()
        ...
        val subsequentSessions = usageSessionDao.getSessionsInRange(
            outcome.targetApp,
            outcome.timestamp,
            fiveMinutesAfter
        )
        ...
        val allSessionsIn30Min = usageSessionDao.getSessionsInRange(
            outcome.targetApp,
            outcome.timestamp,
            thirtyMinutesAfter
        )
    ...
    private suspend fun collectMediumTermOutcome(
        ...
        val sessionsToday = usageSessionDao.getSessionsInRange(
            outcome.targetApp,
            dayStart,
            dayEnd
        )
        ...
        val additionalSessionsToday = sessionsToday.count { it.startTime > outcome.timestamp }

        // Calculate total usage for the day
        val totalUsageToday = sessionsToday
            .filter { it.isEnded && it.endTime != null }
            .sumOf { (it.endTime!! - it.startTime) / (60 * 1000) }  // Convert to minutes
    ...
    private suspend fun collectLongTermOutcome(
        ...
        val sessionsAfter = usageSessionDao.getSessionsInRange(
            outcome.targetApp,
            sevenDaysAfterStart,
            sevenDaysAfterEnd
        )
        ...
        val totalUsageNext7Days = sessionsAfter
            .filter { it.isEnded && it.endTime != null }
            .sumOf { (it.endTime!! - it.startTime) / (60 * 1000) }  // Minutes
        ...
        val sessionsBefore = usageSessionDao.getSessionsInRange(
            outcome.targetApp,
        ...
        val totalUsagePrevious7Days = sessionsBefore
            .filter { it.isEnded && it.endTime != null }
            .sumOf { (it.endTime!! - it.startTime) / (60 * 1000) }
...
private fun ComprehensiveOutcomeEntity.toDomainModel(): ComprehensiveInterventionOutcome {
    return ComprehensiveInterventionOutcome(
        ...
        targetApp = targetApp,
        timestamp = timestamp,
        immediateChoice = UserChoice.valueOf(immediateChoice),
        responseTime = responseTime,
